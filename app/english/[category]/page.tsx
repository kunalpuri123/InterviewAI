...
import { englishQuestions, getEnglishQuestionsByCategory } from "@/lib/aptitude-questions"
...
export default function EnglishCategoryPage() {
  ...
  const actualCategory = Object.keys(englishQuestions.reduce((acc, q) => ({ ...acc, [q.category]: true }), {})).find(
    (cat) => cat.toLowerCase().replace(/\s+/g, "-") === category.toLowerCase(),
  )

  const categoryQuestions = actualCategory ? getEnglishQuestionsByCategory(actualCategory) : []
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        <Button variant="outline" onClick={() => router.push("/english")}>
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back to English
        </Button>
      </div>
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mb-6">
          Practice {actualCategory.toLowerCase()} problems to improve your English language skills.
        </p>
